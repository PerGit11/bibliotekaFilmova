...
export const MovieDetail = () => {
    ...
    useEffect(() => {
        document.title= `${movie.title} / Biblioteka Filmova`;
    });

    useEffect(() => {
        async function fetchMovie(){
            const response = await fetch(`https://api.themoviedb.org/3/movie/${params.id}?api_key=c1262181314a18956f97250f803e9813`);
    ...
    return (
        <main>
            <section className={"flex justify-around flex-wrap py-5"}>
                ...
                <div className={"max-w-2xl text-gray-700 text-lg dark:text-slate-50"}>
                    <h1 className={"text-4xl font-bold my-3 text-center lg:text-left"}>{movie.title}</h1>
                    <p className={"my-4"}>{movie.overview}</p>
                    <p className={"my-7 flex flex-wrap gap-2"}>
                        {movie.genres?.map((genre) => (
                            <span className="mr-2 border border-gray-200 dark:border-gray-600 rounded p-2"
                    ...
                    <p className={"my-4"}>
                        ...
                        <Link to={`https://www.imdb.com/title/${movie.imdb_id}`}>{movie.imdb_id} </Link>
                    </p>
                </div>
            </section>
        </main>
